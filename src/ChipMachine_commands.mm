...
#include "spmediakeytap/SPMediaKeyTap.h"
...
static ChipMachineDelegate* delegate;
SPMediaKeyTap* keyTap;
	
void ChipMachineDelegate::mediaKeyTapReceivedMediaKeyEvent(SPMediaKeyTap* keyTap, NSEvent* event) {
	...
	if (keyState == 1) {
		...
		switch (keyCode) {
			...
			mChipMachine->playPause();
			return;

			case NX_KEYTYPE_FAST:
			mChipMachine->nextSong();
			return;
...
void ChipMachine::setupMediaKeyTap() {
	...
	// setup MediaKeyTap to fetch media key events on mac
	delegate = (ChipMachineDelegate*)new ChipMachineDelegate();
	delegate->mChipMachine = this;
	
	keyTap = [[SPMediaKeyTap alloc] initWithDelegate:((void*)delegate)];
